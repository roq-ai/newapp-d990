...
import { getRolePermissions } from 'apiSdk/role-permissions';
...
function DashboardPage() {
  ...
  const rolePermissionStat = useSWR<PaginatedInterface<RolePermissionInterface>>(
    () => `/role-permissions`,
    () => getRolePermissions({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: rolePermissionStat?.data?.totalCount,
      isLoading: rolePermissionStat.isLoading,
